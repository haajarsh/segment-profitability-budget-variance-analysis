...
#
# All values are in MYR '000 (thousands). So 12500 = RM 12.5 million.
# -----------------------------------------------------------------------------
        ...
        for entity_code, accounts_dict in BASELINE_JAN_2023.items():
            ...
            for account_code, baseline_value in accounts_dict.items():
                ...
                rows.append({
                    ...
                    "amount_myr_thousands": round(actual_value, 2)
                })
                ...
                rows.append({
                    ...
                    "amount_myr_thousands": round(budget_value, 2)
                })
                ...
                rows.append({
                    ...
                    "amount_myr_thousands": round(forecast_value, 2)
                })
...
FX_EXPOSURE = {
    "MAC": 180,  # Aviation Cargo — international shipments
    "MIS": 60,   # Inflight Services — some imported food/equipment
    "MLG": 220,  # Logistics — international freight
    "MMR": 380,  # Marine — heavy USD exposure
}
            ...
            for scenario_key in [1, 2, 3]:  # Actual, Budget, Forecast
                ...
                if entity_code in FX_EXPOSURE:
                    ...
                    if fx_value != 0:
                        new_rows.append({
                            ...
                            "amount_myr_thousands": round(fx_value, 2),
                        })
                ...
                if pbt_with_fx > 0:
                    ...
                    new_rows.append({
                        ...
                        "scenario_key": scenario_key,
                        "amount_myr_thousands": round(tax_amount, 2),
                    })
...
def main():
    ...
    snapshot = (
        ...
        .groupby("account_category")["amount_myr_thousands"].sum()
        .round(0)
    )
    ...
    print(f"  Total costs:     RM {total_cost:>12,.0f}k")
    print(f"  Implied EBIT:    RM {ebit:>12,.0f}k  ({'loss' if ebit < 0 else 'profit'})")
